...
int $DTYPE$_dec_a($DTYPE$ input[], index_t input_len,
    ...
    return $DTYPE$_downsampling_convolution(input, input_len,
                                            wavelet->dec_lo_$DTYPE$,
...
int $DTYPE$_rec_a($DTYPE$ coeffs_a[], index_t coeffs_len,
    ...
    return $DTYPE$_upsampling_convolution_full(coeffs_a, coeffs_len,
                                               wavelet->rec_lo_$DTYPE$,
...
int $DTYPE$_rec_d($DTYPE$ coeffs_d[], index_t coeffs_len,
    ...
    return $DTYPE$_upsampling_convolution_full(coeffs_d, coeffs_len,
                                               wavelet->rec_hi_$DTYPE$,
                                               wavelet->rec_len, output,
                                               output_len);
}
...
int $DTYPE$_idwt($DTYPE$ coeffs_a[], index_t coeffs_a_len,
     ...
     * cleared) memset(output, 0, output_len * sizeof($DTYPE$));
     */

    /* reconstruct approximation coeffs with lowpass reconstruction filter */
    if(coeffs_a){
        if($DTYPE$_upsampling_convolution_valid_sf(coeffs_a, input_len,
                                                   wavelet->rec_lo_$DTYPE$,
                                                   wavelet->rec_len, output,
                                                   output_len, mode) < 0){
            goto error;
     ...
     */
    if(coeffs_d){
        if($DTYPE$_upsampling_convolution_valid_sf(coeffs_d, input_len,
                                                   wavelet->rec_hi_$DTYPE$,
                                                   wavelet->rec_len, output,
                                                   output_len, mode) < 0){
            goto error;
...
int $DTYPE$_swt_($DTYPE$ input[], index_t input_len,
                 ...
                 $DTYPE$ output[], index_t output_len, int level){
    ...
    if(level > 1){
        ...
        ret = $DTYPE$_downsampling_convolution(input, input_len, e_filter,
        ...
        return $DTYPE$_downsampling_convolution(input, input_len, filter,
                                                filter_len, output, 1,
                                                MODE_PERIODIZATION);
    }
}
 ...
 */
int $DTYPE$_swt_a($DTYPE$ input[], index_t input_len, Wavelet* wavelet,
                  $DTYPE$ output[], index_t output_len, int level){
    return $DTYPE$_swt_(input, input_len, wavelet->dec_lo_$DTYPE$,
 ...
 * input - approximation coeffs from upper level or signal if level == 1
 */
int $DTYPE$_swt_d($DTYPE$ input[], index_t input_len, Wavelet* wavelet,
                  $DTYPE$ output[], index_t output_len, int level){
    return $DTYPE$_swt_(input, input_len, wavelet->dec_hi_$DTYPE$,
                        wavelet->dec_len, output, output_len, level);
}
