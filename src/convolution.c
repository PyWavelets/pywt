...
int downsampling_convolution_periodization(CONST_DTYPE* input, const_index_t N,
                             ...
                             const_index_t step, MODE mode)
{
    ...
    if(F <= N){
        ...
        if(mode == MODE_PERIODIZATION){
            return downsampling_convolution_periodization(input, N, filter, F, output, step);
...
int upsampling_convolution_valid_sf_periodization(CONST_DTYPE* input, const_index_t N,
                                    const double* filter, const_index_t F,
                                    DTYPE* output, const_index_t O)
{
