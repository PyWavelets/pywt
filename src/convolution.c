                                        ...
                                        const_index_t step, MODE mode)
{
    ...
    if(mode != MODE_PERIODIZATION){
        ...
        buffer = wtcalloc(N_extended_len, sizeof(double));
        if(buffer == NULL)
            return -1;
                                    ...
                                    double* output, const_index_t O)
{
    ...
    // =======
    {
        ...
        periodization_buf = wtcalloc(N_p, sizeof(double));
        ...
        // The F_2%2==0 case needs special result fix (oh my, another one..)
            ...
            ptr_out = wtcalloc(idwt_buffer_length(N, F, MODE_PERIODIZATION), sizeof(double));
            if(ptr_out == NULL){
    ...
    // Otherwise (N >= F_2)
        ...
        // =======
            ...
            // whole input
            periodization_buf = wtcalloc(N_p, sizeof(double));
            periodization_buf_rear = wtcalloc(N_p, sizeof(double));
                                        ...
                                        const_index_t step, MODE mode)
{
    ...
    if(mode != MODE_PERIODIZATION){
        ...
        buffer = wtcalloc(N_extended_len, sizeof(float));
        if(buffer == NULL)
            return -1;
                                    ...
                                    float* output, const_index_t O)
{
    ...
    // =======
    {
        ...
        // periodization_buf will hold periodically copied input coeffs values
        periodization_buf = wtcalloc(N_p, sizeof(float));
        ...
        // The F_2%2==0 case needs special result fix (oh my, another one..)
            ...
            ptr_out = wtcalloc(idwt_buffer_length(N, F, MODE_PERIODIZATION), sizeof(float));
            if(ptr_out == NULL){
                wtfree(periodization_buf);
                return -1;
            }
    ...
    // Otherwise (N >= F_2)
        ...
        // =======
            ...
            // whole input
            periodization_buf = wtcalloc(N_p, sizeof(float));
            periodization_buf_rear = wtcalloc(N_p, sizeof(float));
...
int short_downsampling_convolution_periodization(const short* input, const_index_t N,
                             const short* filter, const_index_t F,
                             short* output, const_index_t step)
...
int short_downsampling_convolution(const short* input, const_index_t N,
                             const short* filter, const_index_t F,
                             short* output,
                             const_index_t step, MODE mode)
{
    ...
    if(F <= N){
        ...
        if(mode == MODE_PERIODIZATION){
            return short_downsampling_convolution_periodization(input, N, filter, F, output, step);
        ...
        // reallocating memory for short signals (shorter than filter) is cheap
        return short_allocating_downsampling_convolution(input, N, filter, F, output, step, mode);
    }
}
...
int short_allocating_downsampling_convolution(const short* input, const_index_t N,
                                        const short* filter, const_index_t F,
                                        short* output,
                                        const_index_t step, MODE mode)
{
    ...
    if(mode != MODE_PERIODIZATION){
        ...
        buffer = wtcalloc(N_extended_len, sizeof(short));
        if(buffer == NULL)
            return -1;

        memcpy(buffer+F_minus_1, input, sizeof(short) * N);
        stop = N_extended_len;
        ...
        memcpy(buffer+F/2-1, input, sizeof(short) * N);
    ...
    switch(mode){
        ...
        case MODE_ZEROPAD:
        default:
            //memset(buffer, 0, sizeof(short)*F_minus_1);
            //memset(buffer+N_extended_right_start, 0, sizeof(short)*F_minus_1);
            //memcpy(buffer+N_extended_right_start, buffer, sizeof(short)*F_minus_1);
            break;
    }
...
int short_upsampling_convolution_full(const short* input, const_index_t N,
                                const short* filter, const_index_t F,
                                short* output, const_index_t O){
    register index_t i;
    register index_t j;
    short *ptr_out;
...
int short_upsampling_convolution_valid_sf_periodization(const short* input, const_index_t N,
                                    const short* filter, const_index_t F,
                                    short* output, const_index_t O)
{
    
    short *ptr_out = output;
    short *filter_even, *filter_odd;
    short *periodization_buf = NULL;
    short *periodization_buf_rear = NULL;
    short *ptr_base;
    short sum_even, sum_odd;
    index_t i, j, k, N_p = 0;
    index_t F_2 = F/2;
    ...
    // =======
    {
        ...
        // periodization_buf will hold periodically copied input coeffs values
        periodization_buf = wtcalloc(N_p, sizeof(short));
        ...
        // Now perform the valid convolution
        if(F_2%2){
            short_upsampling_convolution_valid_sf(periodization_buf, N_p, filter, F, output, O, MODE_ZEROPAD);
        ...
        // The F_2%2==0 case needs special result fix (oh my, another one..)
            ...
            ptr_out = wtcalloc(idwt_buffer_length(N, F, MODE_PERIODIZATION), sizeof(short));
            if(ptr_out == NULL){
                wtfree(periodization_buf);
                return -1;
            }
            ...
            // Convolve here as for (F_2%2) branch above
            short_upsampling_convolution_valid_sf(periodization_buf, N_p, filter, F, ptr_out, O, MODE_ZEROPAD);
    ...
    // Otherwise (N >= F_2)
        ...
        // Allocate memory for even and odd elements of the filter
        filter_even = wtmalloc(F_2 * sizeof(short));
        filter_odd = wtmalloc(F_2 * sizeof(short));
        ...
        // =======
            ...
            // whole input
            periodization_buf = wtcalloc(N_p, sizeof(short));
            periodization_buf_rear = wtcalloc(N_p, sizeof(short));
            ...
            // Fill buffers with appriopriate elements
            memcpy(periodization_buf + N_p - k, input, k * sizeof(short));        // copy from beginning of input to end of buffer
            for(i = 1; i <= (N_p - k); ++i)                                        // kopiowanie 'cykliczne' od koñca input 
                periodization_buf[(N_p - k) - i] = input[N - (i%N)];

            memcpy(periodization_buf_rear, input + N - k, k * sizeof(short));    // copy from end of input to begginning of buffer
            for(i = 0; i < (N_p - k); ++i)                                        // kopiowanie 'cykliczne' od pocz¹tku input
                periodization_buf_rear[k + i] = input[i%N];
            ...
            if(k%2 == 1){
                ...
                *(ptr_out++) += sum_odd;
                ...
                --k;
                if(k)
                    short_upsampling_convolution_valid_sf(periodization_buf + 1, N_p-1, filter, F, ptr_out, O-1, MODE_ZEROPAD);
        ...
        // =======
            ...
            if(k%2 == 1){
                if(F/2 <= N_p - 1){ // k > 1 ?
                    short_upsampling_convolution_valid_sf(periodization_buf_rear , N_p-1, filter, F, ptr_out, O-1, MODE_ZEROPAD);
                }
                ...
                if(k){
                    short_upsampling_convolution_valid_sf(periodization_buf_rear, N_p, filter, F, ptr_out, O, MODE_ZEROPAD);
                }
            }
        }
        if(periodization_buf != NULL) wtfree(periodization_buf);
...
int short_upsampling_convolution_valid_sf(const short* input, const_index_t N,
                                    const short* filter, const_index_t F,
                                    short* output, const_index_t O,
                                    MODE mode){
    ...
    short sum_even, sum_odd;
    #ifdef OPT_UNROLL2
        short sum_even2, sum_odd2;
    #endif
    #ifdef OPT_UNROLL4
        #ifndef OPT_UNROLL2
            short sum_even2, sum_odd2;
        #endif
        short sum_even3, sum_odd3;
        short sum_even4, sum_odd4;
    #endif
    ...
    if(mode == MODE_PERIODIZATION) // Special case
        return short_upsampling_convolution_valid_sf_periodization(input, N, filter, F, output, O);
...
// -> swt - todo
int short_upsampled_filter_convolution(const short* input, const_index_t N, const short* filter, const_index_t F, short* output, const_index_t step, MODE mode)
{
    return -1;
}
