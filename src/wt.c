...
int short_dec_a(short input[], index_t input_len,
    ...
    return short_downsampling_convolution(input, input_len, wavelet->dec_lo_short,
                                             wavelet->dec_len, output, 2, mode);
}
...
int short_dec_d(short input[], index_t input_len,
    ...
    return short_downsampling_convolution(input, input_len, wavelet->dec_hi_short,
                                             wavelet->dec_len, output, 2, mode);
}
...
int short_rec_a(short coeffs_a[], index_t coeffs_len,
    ...
    return short_upsampling_convolution_full(coeffs_a, coeffs_len, wavelet->rec_lo_short,
                                                wavelet->rec_len, output, output_len);
}
...
int short_rec_d(short coeffs_d[], index_t coeffs_len,
    ...
    return short_upsampling_convolution_full(coeffs_d, coeffs_len, wavelet->rec_hi_short,
                                                wavelet->rec_len, output, output_len);
}
...
int short_idwt(short coeffs_a[], index_t coeffs_a_len,
    ...
    // // set output to zero (this can be ommited if output array is already cleared)
    // memset(output, 0, output_len * sizeof(short));

    // reconstruct approximation coeffs with lowpass reconstruction filter
    if(coeffs_a){
        if(short_upsampling_convolution_valid_sf(coeffs_a, input_len, wavelet->rec_lo_short,
                                wavelet->rec_len, output, output_len, mode) < 0){
            goto error;
        }
    }
    // and add reconstruction of details coeffs performed with highpass reconstruction filter
    if(coeffs_d){
        if(short_upsampling_convolution_valid_sf(coeffs_d, input_len, wavelet->rec_hi_short,
                                wavelet->rec_len, output, output_len, mode) < 0){
            goto error;
        }
    }
...
// basic SWT step
// TODO: optimize
INLINE int short_swt_(short input[], index_t input_len,
                          const short filter[], index_t filter_len,
                          short output[], index_t output_len,
                          int level){
    ...
    if(level > 1){
        // allocate filter first
        e_filter_len = filter_len << (level-1);
        e_filter = wtcalloc(e_filter_len, sizeof(short));
        if(e_filter == NULL)
            return -1;
        ...
        i = short_downsampling_convolution(input, input_len, e_filter, e_filter_len, output, 1, MODE_PERIODIZATION);
        wtfree(e_filter);
        return i;

    } else {
        return short_downsampling_convolution(input, input_len, filter, filter_len, output, 1, MODE_PERIODIZATION);
    }
}

// Approximation at specified level
// input    - approximation coeffs from upper level or signal if level == 1
int short_swt_a(short input[], index_t input_len, Wavelet* wavelet, short output[], index_t output_len, int level){
    return short_swt_(input, input_len, wavelet->dec_lo_short, wavelet->dec_len, output, output_len, level);
}

// Details at specified level
// input    - approximation coeffs from upper level or signal if level == 1
int short_swt_d(short input[], index_t input_len, Wavelet* wavelet, short output[], index_t output_len, int level){
    return short_swt_(input, input_len, wavelet->dec_hi_short, wavelet->dec_len, output, output_len, level);
}
