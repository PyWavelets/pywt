                             ...
                             const_index_t step, MODE mode)
{
    ...
    if(F <= N){
        ...
        return $DTYPE$_allocating_downsampling_convolution(input, N, filter, F,
                                                           output, step, mode);
    }
}
...
int $DTYPE$_upsampling_convolution_valid_sf_periodization(const $DTYPE$* input,
                                                          const_index_t N,
                                                          const $DTYPE$*
                                                          ...
                                                          const_index_t O)
{
    ...
    /* Otherwise (N >= F_2) */
        ...
        if(N_p > 0){
            ...
            memcpy(periodization_buf + N_p - k, input, k * sizeof($DTYPE$));
            for(i = 1; i <= (N_p - k); ++i)
                periodization_buf[(N_p - k) - i] = input[N - (i%N)];

            /* copy from end of input to beginning of buffer */
            memcpy(periodization_buf_rear, input + N - k, k * sizeof($DTYPE$));  
            for(i = 0; i < (N_p - k); ++i)
            ...
            if(k%2 == 1){
                ...
                *(ptr_out++) += sum_odd;
                ...
                $DTYPE$_upsampling_convolution_valid_sf(periodization_buf, N_p,
                                                        filter, F, ptr_out, O,
                                                        MODE_ZEROPAD);
                ptr_out += k;
            }
        ...
        if(N_p > 0){
            ...
            if(k%2 == 1){
                ...
                if(k){
                    $DTYPE$_upsampling_convolution_valid_sf(periodization_buf_rear,
                                                            N_p, filter, F,
                                                            ptr_out, O,
                                                            MODE_ZEROPAD);
                }
...
int $DTYPE$_upsampling_convolution_valid_sf(const $DTYPE$* input,
                                            const_index_t N, const $DTYPE$*
                                            filter, const_index_t F, $DTYPE$*
    ...
    if(mode == MODE_PERIODIZATION) /* Special case */
        return $DTYPE$_upsampling_convolution_valid_sf_periodization(input, N,
                                                                     filter, F,
                                                                     output, O);
...
/* -> swt - todo */
int $DTYPE$_upsampled_filter_convolution(const $DTYPE$* input,
                                         const_index_t N, const $DTYPE$*
                                         filter, const_index_t F, $DTYPE$*
